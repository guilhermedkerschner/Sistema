...
class MenuManager {
   ...
   /**
    * Gera HTML para um item de menu (com suporte a hierarquia)
    */
   private function generateMenuItemHTML($key, $module, $currentPage, $level = 0) {
       $info = $module['info'] ?? [];
       $menu = $module['menu'] ?? [];
       ...
       if ($hasSubmenu) {
           ...
           foreach ($menu['submenu'] as $subKey => $subItem) {
               $html .= $this->generateSubmenuItemHTML($subKey, $subItem, $currentPage, $level + 1);
           }
           $html .= '</ul>';
       } else {
           // Item simples
   ...
   /**
    * Gera HTML para itens de submenu (com suporte a hierarquia)
    */
   private function generateSubmenuItemHTML($key, $item, $currentPage, $level = 1) {
       $hasSubmenu = !empty($item['submenu'] ?? []);
       $isActive = $this->isSubmenuItemActive($item, $currentPage);
       
       if ($hasSubmenu) {
           // Submenu com sub-submenus
           $html = '<li class="menu-item' . ($isActive ? ' open' : '') . '">';
           $html .= '<a href="#" class="menu-link">';
           $html .= '<span class="menu-icon"><i class="' . ($item['icon'] ?? 'fas fa-circle') . '"></i></span>';
           $html .= '<span class="menu-text">' . ($item['name'] ?? 'Item') . '</span>';
           $html .= '<span class="arrow"><i class="fas fa-chevron-right"></i></span>';
           $html .= '</a>';
           
           // Sub-submenu
           $html .= '<ul class="submenu">';
           foreach ($item['submenu'] as $subSubKey => $subSubItem) {
               $subSubFile = $subSubItem['files']['main'] ?? '';
               $subSubIsActive = $this->isPageActive($subSubFile, $currentPage);
               
               $html .= '<li>';
               $html .= '<a href="' . $subSubFile . '" class="submenu-link' . ($subSubIsActive ? ' active' : '') . '">';
               if (isset($subSubItem['icon'])) {
                   $html .= '<i class="' . $subSubItem['icon'] . '"></i> ';
               }
               $html .= ($subSubItem['name'] ?? 'Subitem');
               $html .= '</a>';
               $html .= '</li>';
           }
           $html .= '</ul>';
           ...
           // Item simples do submenu
           $subFile = $item['files']['main'] ?? '';
   ...
   /**
    * Verifica se um item de submenu está ativo
    */
   private function isSubmenuItemActive($item, $currentPage) {
       // Verificar arquivo principal do item
       if (isset($item['files']['main']) && $this->isPageActive($item['files']['main'], $currentPage)) {
           return true;
       }
       
       // Verificar sub-submenu
       if (isset($item['submenu']) && is_array($item['submenu'])) {
           foreach ($item['submenu'] as $subSubItem) {
               if (isset($subSubItem['files']['main']) && $this->isPageActive($subSubItem['files']['main'], $currentPage)) {
                   return true;
               }
           }
       }
   ...
   /**
    * Verifica se um módulo está ativo (com suporte a hierarquia)
    */
   private function isModuleActive($moduleKey, $currentPage) {
       ...
       // Verificar submenu (incluindo sub-submenus)
       if (isset($module['menu']['submenu']) && is_array($module['menu']['submenu'])) {
           foreach ($module['menu']['submenu'] as $subItem) {
               // Verificar item do submenu
               if (isset($subItem['files']['main']) && $this->isPageActive($subItem['files']['main'], $currentPage)) {
                   return true;
               }
